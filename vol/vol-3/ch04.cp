...
a relatively big step big.  That should make it easier to interpret
the output:

 :commandoutput: ruby rknbody1b_driver.rb < test5.in

*Bob*: But not _that_ easy.  I don't like to compute by hand all the steps
in the fourth-order Runge-Kutta algorithm.  How about checking whether the
forward Euler breaks down as well?  That will be far easier to check by hand.

== Forward Euler

*Alice*: Good idea!  And we may as well make the total integration time a
bit shorter, to speed things up.
...
<tex>$\bv dt = 0.01 \bv$</tex>.  And so they are.
...
<tex>$\ba dt = 0.01 \ba$</tex>.  The acceleration, for particles of
mass 0.9, at distance 1, should be 0.9, which means that <tex>$|\ba dt| =
...
_y_ axis.  Perhaps we should take a really generic initial position,
...
*Bob*: But I've just showed that it did _not_ malfunction!
...
*Alice*: Agreed.  And in order to follow our own advice, let us now test our
...
All it does it to read in an N-body system, and print the relative positions
