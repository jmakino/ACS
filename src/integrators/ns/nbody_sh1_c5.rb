...
# avoid copy from/to Ruby in time integration loop
#
#==============================================================================
...
def get_acc_jerk_pot_coll_in_c2(nb,cnb)
  nb.each do |b| b.cbody.clear_acc_and_jerk end
  nb[0].clear_epot
  nb[0].reset_coll_time_q
  nb[0].cbody.set_epot_and_collq(Body)
...
#    j = i+1
#    while (j < n)
#      cnb[i].pairwize_force(cnb[j])
#      j += 1
#    end
#    i += 1
#  end
  Cbody_calculate_force_all(cnb,n);
...
def evolve_step(nb, cnb, dt)
  nb.each do |b| b.cbody.update_old end
  nb[0].cbody.set_dt(dt)
  nb.each do |b| b.cbody.predict end
  get_acc_jerk_pot_coll_in_c2(nb,cnb)
