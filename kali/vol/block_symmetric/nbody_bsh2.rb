...
class NBody
  ...
  def evolve(c)
    ...
    while @time < t_end
      @dt = olddt
      send(c.method)
      newdt = c.dt_param * collision_time_scale

#      newdt2 = force_power_2(newdt*newdt)
#      olddt = newdt2/olddt
#      newdt = sqrt(newdt2)
#      olddt = exp(log(newdt)+(log(olddt)-log(newdt))*0.9)
#      olddt = newdt

      olddt = 1.0/(2.0/newdt - 1.0/olddt)
#      olddt = (2.0*newdt - olddt)

#      print "dt = #{sqrt(newdt2)},  #{olddt}\n"
      @time += @dt
      @nsteps += 1
      istep += 1
