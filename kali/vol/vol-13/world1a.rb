...
class Worldpoint < Body
  ...
  def ms4_correct(wa, acc, timescale, dt_param, dt_max)
    ...
    @acc, @jerk, @snap, @crackle, @pop = a
    dt = wa[0].time - @time
    @vel = wa[0].vel - taylor_increment(a, dt)
    @pos = wa[0].pos - taylor_increment([@vel, *a], dt)
    dt = timescale * dt_param
    dt = dt_max if dt > dt_max
    @next_time = @time + dt
    admin(wa[0].time)
...
#    taylor_pos = dt*( @vel + (1/2.0)*dt*( @acc + (1/3.0)*dt*( @jerk + (1/4.0)*dt*( @snap + (1/5.0)*dt*( @crackle + (1/6.0)*dt*@pop )))))
#    taylor_vel = dt*( @acc + (1/2.0)*dt*( @jerk + (1/3.0)*dt*( @snap + (1/4.0)*dt*( @crackle + (1/5.0)*dt*@pop ))))
...
#  def taylor_increment(a, dt, number = 1)
#    if number >= a.size
#      return dt*a[number-1]
#    else
#      dt*(a[number-1] + (1.0/(number+1))*taylor_increment(a, dt, number+1))
#    end
#  end
...
options_text= <<-END
  ...
  Long description:
    ...
    This is a test version for developing an arbitrary-order generalization
    of Aarseth's scheme
