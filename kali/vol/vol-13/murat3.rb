...
class Body
  ...
  attr_accessor :mass, :pos, :vel, :old_pos, :old_vel, :old_acc, :old_jerk,
  ...
  def set_next_update(ba, dt_param)
    @next_update = @last_update + collision_time_scale(ba) * dt_param
  end
  ...
  def sync(ba, t, dt_param)
    @next_update = t
    take_one_step(ba, dt_param)
  end

  def take_one_step(ba, dt_param)
    ba.each do |b|
      b.predict_step(@next_update)
    end
    correct_step(ba, @next_update)
    @last_update = @next_update
    set_next_update(ba, dt_param)
  end

  def predict_step(t)
    dt = t - @last_update
    @pos = @old_pos + @old_vel*dt + @old_acc*(dt*dt/2.0) +
                                                       @old_jerk*(dt*dt*dt/6.0)
    @vel = @old_vel + @old_acc*dt + @old_jerk*(dt*dt/2.0)
  end

  def correct_step(ba, t)
    dt = t - @last_update
    new_acc = acc(ba)
    new_jerk = jerk(ba)
    @vel = @old_vel + (@old_acc + acc(ba))*(dt/2.0) +
                      (@old_jerk - jerk(ba))*(dt*dt/12.0)
    @pos = @old_pos + (@old_vel + @vel)*(dt/2.0) +
...
class Nbody
  ...
  def init_update(dt_param)
    @body.each do |b|
      b.last_update = @time
      b.next_update = b.set_next_update(@body, dt_param)
    end
  end
  ...
  def init_acc_jerk
    @body.each do |b|
      b.old_pos = b.pos
      b.old_vel = b.vel
      b.old_acc = b.acc(@body)
      b.old_jerk = b.jerk(@body)
    end
  end
  ...
  def find_next_particle
    ...
    next_particle = nil
    @body.each do |b|
      if next_time > b.next_update
        next_time = b.next_update
        next_particle = b
      end
    end
    next_particle
  ...
  def sync(t, dt_param)
    @body.each{|b| b.sync(@body, t, dt_param)}
    @time = t
  end
  ...
  def evolve(integration_method, dt_param, delta_dia, delta_out,
    ...
    init_update(dt_param)
    init_acc_jerk
    while @time < t_end
      np = find_next_particle
      @time = np.next_update        # is this really the best way ????
      if (@time < t_end)
        np.take_one_step(@body, dt_param)
        nsteps += 1
      end
      if @time >= t_dia
