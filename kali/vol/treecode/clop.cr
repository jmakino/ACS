...
class Clop_Option
  ...
  getter :shortname, :longname, :type,
  ...
  def parse_single_lines_done?(s)
    ...
    name = $1
    startloc = s.index(name)
    content = s[(startloc+name.length)..(s.length)]
    case name
...
#        i = x if s =~ Regexp.new(@options[x].shortname) && $` == ""         #4
         i = x if s ==@options[x].shortname
      end
    end
    return i
  end
