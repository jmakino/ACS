...
run_seed = c.seed
c.n_runs.times do
  t = `kali mkplummer.rb -n #{c.n} -s #{run_seed} --verbosity #{c.run_verbosity} | kali world4.rb -t #{c.max_run_time} -x #{c.max_semi_major_axis} --verbosity #{c.run_verbosity} | kali test2.rb --verbosity 0`.chomp
  if t.size == 0
