...
class Body
  ...
  def corr_acc(body_array, eps)
    ga = @pos*0                             # null vector of the correct length
    sa = self.acc(body_array, eps)
    body_array.each do |b|
      unless b == self
        ba = b.acc(body_array, eps)
        r = b.pos - @pos
        r2 = r*r + eps*eps
        r3 = r2*sqrt(r2)
        a = ba - sa
...
class NBody
  ...
  def cc
    calc(" @old_pos = @pos ")
    calc(" @a0 = acc(ba,eps) ")
    calc(" @pos = @old_pos + 0.5*@vel*dt + (1.0/12)*@a0*dt**2 ")
    calc(" @a1 = acc(ba,eps) + (1/48.0)*corr_acc(ba,eps)*dt**2 ")
    calc(" @pos = @old_pos + @vel*dt + (1/6.0)*(@a0+2*@a1)*dt**2 ")
    calc(" @a2 = acc(ba,eps) ")
    calc(" @vel += (1/6.0)*(@a0+4*@a1+@a2)*dt ")
  end
