...
class VirtualPlotter
  ...
  def get_plotter_from_env(envname)
    if ENV[envname] != nil
      plotter, file = ENV[envname].split(":")
    else
      plotter,file = ["gnuplot",nil]
    end
    [plotter,file]
  end
  
  def initialize(plotter=nil, output_file=nil)
    if plotter == nil
      plotter,output_file=get_plotter_from_env("VP_PLOTTER")
    end
    @plotter = plotter    
