...
#    ins.each{|x| p x}
  ...
  def latex_process_tex_weblinks(instring)
	...
	text=$2
        ...
        url = escapetexspecialcharacters(linktext)
        text = linktext
        s="<tex>{\\tt #{text}\\footnote{\\tt #{url}}}</tex>"
      end
      s
    }
...
#    ostring.split("\n")
  ...
  def latex_process_citations(instring)
    ins = instring.join("\n")
    ostring=ins.gsub(/<cite([pt])>(.+?)<\/cite[pt]>/m){ 
      citetype = $1
      citetag = $2
      "<tex>\\cite#{citetype}{#{citetag}}</tex>"
    }.split("\n")
  end
...
#	p ss
  ...
  def process_include(instring, tolatex_mode)
    ostring=[] 
    while s=instring.shift
      if /^(\s*)\:include\:\s*(\S+)$/  =~ s
        indent = $1
...
#
  def command_with_input(s,tag, fname,lineno,instring, 
                         dirname, showout, showin,saveresult )
    ostring = ""
    a = s.chomp.split
    endtag = a.pop
    indata=""
...
# 
	...
	ostring = ostring +  command_with_input(s,":commandinput:", 
                                                fname,lineno,instring,
...
@@reference_tag_table = []
  ...
  def convert_to_single_reference(s)
    authors, year, tag, refdata  = *s.split("|")
    if refdata == nil
      retval=s
    else
      authors = authors.split(";")
      shortauthors = Rdoctotex::authorsforcite(authors)
      @@reference_tag_table.push([tag, "#{shortauthors}(#{year})",
                                   "(#{shortauthors},#{year})"])
      retval = "<p>#{Rdoctotex::authorsforref(authors)}, #{year}, #{refdata}</p>\n"
    end
    retval
  end

  def convert_citations(s)
    @@reference_tag_table.each{|ref|
      s.gsub!(Regexp.new("<cite([pt])>(\s*)#{ref[0]}(\s*)<\/cite[pt]>")){
        if $1 == "t"
          ref[1]
        else
          ref[2]
  ...
  def convert_to_reference(instring)
...
END
  ...
  def process_references(instring)
    instring.gsub(/<REF>(.+?)<\/REF>/m){ 
      convert_to_reference($1)
    }
  end
  ...
  def prep_cp(infile, outfile,tolatex_flag)
    ...
    if tolatex_flag
      ...
      tmp2= Rdoctotex::process_include(tmp2,false)
